...
class RSCode:
    def __init__(self, m, t, l, m0):
        ...
        self.g = self.makeGenerator(
            m, t, m0
        )  # generator polynomial represented by a galois.Poly variable
    ...
    def encode(self, msg):
        ...
        assert (
            np.shape(msg)[1] == self.l
        ), "the number of columns must be equal to self.l"
        assert type(msg) is galois.GF(
            2**self.m
        ), "each element of msg  must be a galois.GF element"
        ...
        n_par = self.n - self.k
        pad = self.k - self.l
        code = GF.Zeros((msg.shape[0], self.l + n_par))
        for r in range(msg.shape[0]):
            # shortened -> full message (k symbols)
            m_full = np.concatenate((GF.Zeros(pad), msg[r, :]))
            # message polynomial (descending powers)
            M = galois.Poly(m_full, field=GF)
            # x^(n-k)
            x_nk = galois.Poly([1] + [0] * n_par, field=GF)
            # parity remainder
            R = (M * x_nk) % self.g
            # full-length systematic codeword
            C = M * x_nk + R
            c_full = C.coeffs
            # ensure exact length n (pad leading zeros if needed)
            if c_full.size < self.n:
                c_full = np.concatenate((GF.Zeros(self.n - c_full.size), c_full))
            # shorten again: drop first (k-l) symbols
            code[r, :] = c_full[pad:]

        assert (
            np.shape(code)[1] == self.l + self.n - self.k
        ), "the number of columns must be equal to self.l+self.n-self.k"
        assert type(code) is galois.GF(
            2**self.m
    ...
    def decode(self, code):
        ...
        assert (
            np.shape(decoded)[1] == self.l
        ), "the number of columns must be equal to self.l"
        assert type(decoded) is galois.GF(
            2**self.m
        ), "each element of decoded  must be a galois.GF element"
        assert (
            type(nERR) is np.ndarray and len(np.shape(nERR)) == 1
        ), "nERR must be a 1D numpy array"
    ...
    def makeGenerator(m, t, m0):
        ...
        generator = galois.Poly([1], field=GF)  # 1
        for i in range(2 * t):
            generator *= x - galois.Poly([alpha ** (m0 + i)], field=GF)

        assert type(generator) == type(
            galois.Poly([0], field=galois.GF(2**m))
        ), "generator must be a galois.Poly object"
        return generator
    ...
    def test():
        ...
        msg = galois_field(
            np.random.randint(0, 2**8 - 1, (5, 10))
        )  # Generate a random message of 5 information words
